...
public class HomeSearchServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		isTrue = HomeSearchDBUtil.validate(stid);

		if (isTrue == true) {
			List<Results> rDetails = HomeSearchDBUtil.getResults(stid);
			request.setAttribute("rDetails", rDetails);
			
			RequestDispatcher dis = request.getRequestDispatcher("showResultsHome.jsp");
